"""
Red Queen batch experiment — human messages only.

Same batching as red_queen_batch.py; assistant text is omitted before analysis.

Usage::

    python experiments/red_queen_batch_human_only.py
    python experiments/red_queen_batch_human_only.py --batch 1 --sample 5
    python experiments/red_queen_batch_human_only.py --provider ollama --model gemma4:latest
"""
...
def experiment_output_path(model_name: str, sample: int, batch: int) -> Path:
    ...
    return OUTPUT_DIR / f"batch_{batch:04d}_human_only_n{sample}_{slug}_{stamp}.csv"


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Red Queen batch + TCA analysis (human messages only).",
    )
    ...
    parser.add_argument("--allow-repeat-rows", action="store_true")
    return parser
...
def main() -> None:
    ...
    with tee_terminal_output(log_path):
        print("Red Queen batch experiment (human messages only)")
