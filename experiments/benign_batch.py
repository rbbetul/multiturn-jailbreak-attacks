"""
Benign false-positive batch experiment.

Runs TCA on benign ChatGPT conversations. Expect mostly ``allow`` decisions;
any ``warn`` or ``block`` is a potential false positive.
...
Then run a 5-conversation batch::
    ...
    python experiments/benign_batch.py
    python experiments/benign_batch.py --batch 1 --provider ollama --model gemma4:latest
"""
...
def experiment_output_path(model_name: str, sample: int, batch: int) -> Path:
    ...
    return OUTPUT_DIR / f"benign_batch_{batch:04d}_n{sample}_{slug}_{stamp}.csv"


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Benign ChatGPT batch + TCA false-positive check."
    )
...
def main() -> None:
    ...
    with tee_terminal_output(log_path):
        print("Benign false-positive batch experiment")
