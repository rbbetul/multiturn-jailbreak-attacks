"""
Benign false-positive batch experiment — human messages only.

Same batching as benign_batch.py; assistant text is omitted before analysis.

Build a subset first (full export is too large to load at once)::

    python dataset/benign.py --preset standard
    python dataset/benign.py --sample 100 --output dataset/benign_subset100.csv

Then run a 5-conversation batch::

    python experiments/benign_batch_human_only.py
    python experiments/benign_batch_human_only.py --batch 1 --provider ollama --model gemma4:latest
"""
...
def experiment_output_path(model_name: str, sample: int, batch: int) -> Path:
    ...
    return OUTPUT_DIR / f"benign_batch_{batch:04d}_human_only_n{sample}_{slug}_{stamp}.csv"


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Benign ChatGPT batch + TCA false-positive check (human messages only).",
    )
    parser.add_argument("--data", default=DATASET, help="Benign subset CSV (chatgpt format)")
...
def main() -> None:
    ...
    with tee_terminal_output(log_path):
        print("Benign false-positive batch experiment (human messages only)")
        print(f"  Dataset:     {data_path}")
        print(f"  Format:      chatgpt (auto)")
